...
| --- | --- |
| `@pure-algebra/harness` | `effect4-check <dir>`: unpatched `tsc`, `@effect/tsgo` strict diagnostics, and a node run of `tail.ts`, all against an exact host pin; `effect4-trace <dir> --golden --masks`: run a traced tail and compare its trace with a Lean golden under every registered mask, writing a receipt |
